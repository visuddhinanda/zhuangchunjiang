...
def launch(corpus,start,end):
    # 每次请求间隔秒数
    delay=1.0

    log.info("download %s start %d end %d", corpus,start,end)
    # args = parse_args()

    if corpus not in CORPUS_CONFIG:
        log.error("未知 corpus: %s，支持: %s", corpus, list(CORPUS_CONFIG))
        sys.exit(1)

    cfg = CORPUS_CONFIG[corpus]
    
    # 确定位数：优先使用命令行参数，否则使用配置文件
    n_digits = cfg.get("n_digits", 0)
    ...
    write_meta(out_dir, corpus, cfg["book_id"], start, end, n_digits)

    if failed:
        log.warning("以下章节下载失败，请手动重试: %s", failed)
        sys.exit(1)
    else:
        log.info("全部 %d 章节下载完毕。", end - start + 1)
