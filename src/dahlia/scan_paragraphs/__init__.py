...
def load_one_row(conn, book_id: int, paragraph: int) -> dict | None:
    result = conn.execute(
        text("SELECT paragraph, text FROM pali_texts WHERE book = :book AND paragraph = :para"),
        {"book": book_id, "para": paragraph},
    )
...
def match_pali_texts(
    ...
    logger.info("  开头验证通过: %s", agama_norm[:head_len])
    ...
    while True:
        ...
        if not row:
            logger.warning("  pali_texts 已耗尽，cursor=%d", cursor)
            break

        candidate     = accumulated + strip_whitespace(row["text"])
        candidate_len = len(candidate)
        logger.debug("  指针=%d 缓冲=%d 目标=%d", cursor, candidate_len, agama_len)
...
def launch(db, corpus: str, start: int | None, end: int | None) -> None:
    logger.info("scan_paragraphs: corpus=%s start=%s end=%s", corpus, start, end)
    ...
    logger.info("共 %d 个 HTML 文件待扫描", len(html_files))
    ...
    if map_path.exists():
        ...
        logger.info("已有 para_map.jsonl，已扫描 %d 条，将跳过", len(existing))
    ...
    with db.connect() as conn:
        if existing:
            ...
            cursor = last_para + 1
            logger.info("续扫游标：paragraph = %d", cursor)
        else:
            result = conn.execute(
                text("SELECT MIN(paragraph) FROM pali_texts WHERE book = :book"),
                {"book": book_id},
            )
            cursor = result.scalar()
            logger.info("游标初始化：paragraph = %d", cursor)

        # ── 主循环 ────────────────────────────────────────────────────────────
        with map_path.open("a", encoding="utf-8") as out_f:
            for html_path in html_files:
                if html_path.name in existing:
                    logger.info("跳过已扫描: %s", html_path.name)
                    continue

                chapter_label = f"{corpus}/{html_path.name}"
                logger.info("══════ 扫描 %s ══════", chapter_label)

                html      = html_path.read_text(encoding="utf-8")
                pali_text = extract_pali(html)

                if not pali_text.strip():
                    logger.warning("%s: #east 为空，跳过", chapter_label)
                    continue
                ...
                logger.info(
                    "  已写入: %s → 段落 %d..%d",
                    html_path.name, matched[0], matched[-1],
                )

    logger.info("扫描完成 → %s", map_path)